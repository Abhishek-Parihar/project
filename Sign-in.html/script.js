...
const typer = document.getElementById("input");
const imageTag = document.getElementById("icon");
...
function handleKeyPress(e) {
  ...
  if (imageTag.getAttribute("src") != "./loading.gif")
    imageTag.setAttribute("src", "./loading.gif");
}
...
function handleKeyUp(e) {
  ...
  timer = window.setTimeout(() => {
    imageTag.setAttribute(
      "src",
      "https://img.icons8.com/ios/50/000000/search--v3.png"
    );
  }, timeoutVal);
}
...
var mySVG = document.querySelector(".svgContainer"),
  ...
  form_inputs = document.querySelector(".signup_form_firebase"),
  signup = document.querySelector("#loginbutton");
...
function getCoordEmail(e) {
  ...
  var eyeLCoords = { x: svgCoords.x + 300, y: svgCoords.y + 76 }; //84
  var eyeRCoords = { x: svgCoords.x + 300, y: svgCoords.y + 76 }; //113
  var noseCoords = { x: svgCoords.x + 280, y: svgCoords.y + 81 };
  var mouthCoords = { x: svgCoords.x + 280, y: svgCoords.y + 100 };
...
function getCoordNamee(e) {
  var carPos = namee.selectionEnd,
    div = document.createElement("div"),
    span = document.createElement("span"),
    copyStyle = getComputedStyle(namee),
    nameeCoords = {},
    caretCoords = {},
    centerCoords = {};
  [].forEach.call(copyStyle, function (prop) {
  ...
  div.style.position = "absolute";
  document.body.appendChild(div);
  div.textContent = namee.value.substr(0, carPos);
  span.textContent = namee.value.substr(carPos) || ".";
  div.appendChild(span);

  nameeCoords = getPosition(namee); //console.log("NameeCoords.x: " + nameeCoords.x + ", NameeCoords.y: " + nameeCoords.y);
  caretCoords = getPosition(span); //console.log("caretCoords.x " + caretCoords.x + ", caretCoords.y: " + caretCoords.y);
  centerCoords = getPosition(mySVG); //console.log("centerCoords.x: " + centerCoords.x);
  svgCoords = getPosition(mySVG);
  screenCenter = centerCoords.x + mySVG.offsetWidth / 2; //console.log("screenCenter: " + screenCenter);
  caretPos = caretCoords.x + nameeCoords.x; //console.log("caretPos: " + caretPos);
...
namee.addEventListener("focus", onNameeFocus);
namee.addEventListener("blur", onNameeBlur);
namee.addEventListener("input", onNameeInput);
...
// var form = document.querySelector(".signup_form_firebase");

// form.addEventListener("submit" , function(e){
//   e.preventDefault();
//   console.log(namee.value, email.value, password.value, confirmPassword.value);
// })
